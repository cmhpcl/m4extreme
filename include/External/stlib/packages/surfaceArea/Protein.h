...
/*!
  \param _Number The floating-point number type.
...
public:
   ...
   typedef std::tr1::array<Number, 2> AreaTuple;
   //! A bounding box.
...
private:
   ...
   array::StaticArrayOfArrays<Rotamer> _rotamers;
   ...
   array::StaticArrayOfArrays<BBox> _rotamerBBoxes;
   ...
   array::StaticArrayOfArrays<AreaTuple> _areaTriPeptideClipped;
   //! The area of the peptide and rotamer, clipped by the full template.
   array::StaticArrayOfArrays<AreaTuple> _areaTemplateClipped;
...
public:
   ...
   */
   Protein(// Reference atoms.
      ...
      assert(sum(aminoAcidSizes) == aminoAcidAtoms.size());
      assert(numRotamersPerSite.size() + 1 == peptideGroups.size());
      assert(sum(numRotamersPerSite) == rotamers.size());
