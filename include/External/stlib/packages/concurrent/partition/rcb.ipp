...
rcbSplit(IDType** partitionBegin, IDType** partitionEnd,
         const std::tr1::array<double, N>** recordsBegin,
         const std::tr1::array<double, N>** recordsEnd) {
   ...
   *partitionMid = *partitionBegin + numRecordsLeft;
   ...
   //
   geom::BBox<N, double> bbox;
   bbox.bound(ads::constructIndirectIterator(recordsBegin),
              ads::constructIndirectIterator(recordsEnd));
   Point extents = bbox.getUpperCorner() - bbox.getLowerCorner();
...
rcb(const std::size_t numProcessors,
    ...
    const std::vector<std::tr1::array<double, N> >& positions) {
   typedef std::tr1::array<double, N> Point;
