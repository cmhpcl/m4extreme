...
structuredGridWriteIndexedSimplexSet(std::ostream& out,
   ...
   for (Index i = 0; i != iEnd; ++i) {
      for (Index j = 0; j != jEnd; ++j) {
         ...
         if (squaredDistance(sg(ext::make_array(i, j)),
                             sg(ext::make_array(i + 1, j + 1))) <
               squaredDistance(sg(ext::make_array(i + 1, j)),
                               sg(ext::make_array(i, j + 1)))) {
            std::cout
                  << sg.getGrid().arrayIndex(ext::make_array(i, j)) << " "
                  << sg.getGrid().arrayIndex(ext::make_array(i + 1, j)) << " "
                  << sg.getGrid().arrayIndex(ext::make_array(i + 1, j + 1)) << '\n'
                  << sg.getGrid().arrayIndex(ext::make_array(i + 1, j + 1)) << " "
                  << sg.getGrid().arrayIndex(ext::make_array(i, j + 1)) << " "
                  << sg.getGrid().arrayIndex(ext::make_array(i, j)) << '\n';
         }
         else {
            std::cout
