...
template < class _QualityMetric = SimplexModCondNum<3>,
         class _Point = std::tr1::array<double, 3>,
...
private:
   ...
   typedef typename array::MultiArray<Number, 2>::Index Index;
...
public:
   ...
   const std::tr1::array<std::size_t, 3>&
   ...
   */
   ...
   setRing(InputIterator begin, InputIterator end) {
      ...
      if (_quality.extents()[1] < _ring.size()) {
         _quality.rebuild(ext::make_array(_ring.size() - 2, _ring.size()));
         _index.rebuild(ext::make_array(_ring.size() - 2, _ring.size()));
      }
   }
