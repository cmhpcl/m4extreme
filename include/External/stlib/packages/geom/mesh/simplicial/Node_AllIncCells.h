...
public:
   ...
   Node_AllIncCells(const vertex_type& vertex, const std::size_t identifier = -1) :
      base_type(vertex, identifier),
      _cells() {}

   //! Build from a vertex and an identifier.
   void
   build(const vertex_type& vertex, const std::size_t identifier = -1,
   ...
   Node_AllIncCells(const vertex_type& vertex,
   ...
   void
   build(const vertex_type& point, const std::size_t identifier = -1,
         const cell_iterator cell = 0) {
   ...
   */
   void
   set_identifier(const std::size_t identifier) const {
   ...
   */
   ...
   put(std::ostream& out) const {
      ...
      const std::size_t i_end = _cells.size();
      // CONTINUE REMOVE
      out << " cells = ";
