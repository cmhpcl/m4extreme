...
build(VertInIter verticesBeginning, VertInIter verticesEnd,
   ...
   std::vector<NodeIterator> nodeHandles(computeNodesSize());
   {
      NodeIterator ni = getNodesBeginning();
      typename std::vector<NodeIterator>::iterator i = nodeHandles.begin();
...
#ifdef DEBUG_stlib
      //assert(ch->getNode(m) != NodeIterator(0));
#endif
      if (ch->getNode(m) != NodeIterator(0)) {
...
buildCellAdjacencies() {
   ...
   typedef std::tr1::array<std::size_t, M> SortedFace;
