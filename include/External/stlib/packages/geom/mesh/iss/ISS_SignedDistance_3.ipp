...
private:
   ...
   std::vector<std::tr1::array<std::size_t, 3> > _incidentEdges;
   //! The face normals.
...
operator()(const Vertex& x, Vertex* closestPoint, Vertex* gradient,
   ...
   for (std::size_t i = 0; i != i_end; ++i) {
      ...
      for (m = 0; m != 3; ++m) {
         ...
         // candidate faces.
         if (_adjacentFaces(n, m) != std::size_t(-1) && n < _adjacentFaces(n, m)
               && faceSet.count(n) == 1) {
            // Make the edge.
