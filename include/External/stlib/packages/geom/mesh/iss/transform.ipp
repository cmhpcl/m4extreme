...
pack(IndSimpSet<N, M, T>* mesh, IntOutputIterator usedVertexIndices) {
   ...
   std::vector<bool> used(mesh->getVerticesSize(), false);
   ...
   std::vector<std::size_t> indices(mesh->getVerticesSize(),
...
removeLowAdjacencies(IndSimpSetIncAdj<N, M, T>* mesh,
   ...
   std::tr1::array < std::size_t, M + 2 > adjacencyCounts;
   std::size_t hi, lo;
   ...
   do {
      ...
      if (ss.size() != mesh->getSimplicesSize()) {
