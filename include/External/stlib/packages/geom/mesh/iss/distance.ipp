...
(const IndSimpSetIncAdj<2, 1, _T>& mesh,
 ...
 typename IndSimpSetIncAdj<2, 1, _T>::Vertex* closestPoint) {
   ...
   std::vector<Number> vertexSquaredDistances(mesh.getVerticesSize());
   ...
   for (std::size_t i = 0; i != mesh.getVerticesSize(); ++i) {
      vertexSquaredDistances[i] =
         squaredDistance(point, mesh.getVertex(i));
   ...
   *closestPoint = mesh.getVertex(closestVertexIndex);
   ...
   for (std::size_t i = 0; i != mesh.getSimplicesSize(); ++i) {
      const std::size_t sourceIndex = mesh.getIndexedSimplex(i)[0];
      const std::size_t targetIndex = mesh.getIndexedSimplex(i)[1];
...
(const IndSimpSetIncAdj<2, 1, _T>& mesh,
 ...
 typename IndSimpSetIncAdj<2, 1, _T>::Vertex* closestPoint) {
   typedef typename IndSimpSetIncAdj<2, 1, _T>::Number Number;
   std::vector<Number> squaredHalfLengths(mesh.getSimplicesSize());
   for (std::size_t i = 0; i != mesh.getSimplicesSize(); ++i) {
      squaredHalfLengths[i] = 0.25 *
                              squaredDistance(mesh.getSimplexVertex(i, 0),
   ...
   for (std::size_t i = 0; i != mesh.getSimplicesSize(); ++i) {
      ...
      if (lowerBound < minimumSquaredDistance) {
         ...
         for (std::size_t n = 0; n != M + 1; ++n) {
            std::size_t adjacentIndex = mesh.getAdjacent(i, n);
...
(const IndSimpSetIncAdj<3, 2, _T>& mesh,
 ...
 typename IndSimpSetIncAdj<3, 2, _T>::Vertex* closestPoint) {
   ...
   std::vector<Number> squaredLongestEdgeLengths(mesh.getSimplicesSize());
   for (std::size_t i = 0; i != mesh.getSimplicesSize(); ++i) {
      squaredLongestEdgeLengths[i] =
         ads::max(squaredDistance(mesh.getSimplexVertex(i, 0),
                                  mesh.getSimplexVertex(i, 1)),
                  squaredDistance(mesh.getSimplexVertex(i, 1),
                                  mesh.getSimplexVertex(i, 2)),
                  squaredDistance(mesh.getSimplexVertex(i, 2),
