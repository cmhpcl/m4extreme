...
PointsOnManifold(const IndSimpSet<N, M, T>& iss,
   ...
   _isAnEdge(ext::make_array<std::size_t>
             (_surfaceManifold.getSimplicesSize(), M + 1)),
...
PointsOnManifold(const IndSimpSet<N, M, T>& iss,
   ...
   if (maxSolidAngleDeviation >= 0) {
      ...
      if (maxBoundaryAngleDeviation >= 0) {
         ...
         for (std::size_t i = 0; i != _surfaceManifold.getVerticesSize(); ++i) {
...
insertAtVertex(const std::size_t pointIdentifier, const std::size_t vertexIndex) {
   ...
   assert(vertexIndex < _surfaceManifold.getVerticesSize());
   ...
   // If the vertex is an edge feature.
      ...
      // An incident simplex.
      const std::size_t simplexIndex = _edgeManifold.getIncident(vertexIndex, 0);
      // Make the point.
      const typename PointMap::value_type
   ...
   // If the vertex is a surface feature.
      ...
      // An incident simplex.
      const std::size_t simplexIndex = _surfaceManifold.getIncident(vertexIndex, 0);
      // Make the point.
      const typename PointMap::value_type
...
insertAtVertices() {
   // For each vertex.
   for (std::size_t n = 0; n != _surfaceManifold.getVerticesSize(); ++n) {
...
insertOnAnEdge(const std::size_t pointIdentifier, const Vertex& position) {
   ...
   assert(simplexIndex < _edgeManifold.getSimplicesSize());
...
insertOnASurface(const std::size_t pointIdentifier, const Vertex& position) {
   ...
   // Check that the simplex index is in the right range.
   assert(simplexIndex < _surfaceManifold.getSimplicesSize());
...
insertBoundaryVerticesAndEdges(IndSimpSetIncAdj < N, M + 1, T > * mesh,
   ...
   for (std::size_t i = 0; i != mesh->getVerticesSize(); ++i) {
...
buildEdgesAndCorners(EdgeInIter edgesBegin, EdgeInIter edgesEnd,
   ...
   //
   {
      ...
      for (; i != edges.end(); ++i, ++j) {
         ...
         {
            array::MultiArray<bool, 2>::IndexList indexList = {{n, m}};
         ...
         if (! _surfaceManifold.isOnBoundary(edge)) {
            ...
            {
               array::MultiArray<bool, 2>::IndexList indexList = {{
                     _surfaceManifold.getAdjacent(n, m),
                     _surfaceManifold.getMirrorIndex(n, m)
                  }
               };
               _isAnEdge(indexList) = true;
            }
   ...
   for (std::size_t i = 0; i != _edgeManifold.getSimplicesSize(); ++i) {
   ...
   // For each vertex.
   for (std::size_t i = 0; i != _edgeManifold.getVerticesSize(); ++i) {
...
isValid(const FaceHandle& face) const {
   ...
   if (face.getFeature() == CornerFeature) {
      return face.getIndex() < _surfaceManifold.getVerticesSize();
   }
   // If it is an edge feature.
   if (face.getFeature() == EdgeFeature) {
      return face.getIndex() < _edgeManifold.getVerticesSize();
   }
   // Else, it is a surface feature
   assert(face.getFeature() == SurfaceFeature);
   return face.getIndex() < _surfaceManifold.getSimplicesSize();
}
