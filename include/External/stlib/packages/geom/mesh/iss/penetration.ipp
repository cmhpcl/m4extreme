...
reportPenetrations(const IndSimpSetIncAdj<N, N, T>& mesh,
   ...
   typedef CellArrayStatic<N, PointRandomAccessIterator> Orq;
   ...
   for (std::size_t simplexIndex = 0; simplexIndex != mesh.getSimplicesSize();
         ++simplexIndex) {
...
maximumIncidentEdgeLength(const IndSimpSetIncAdj<N, N, T>& mesh,
   ...
   for (std::size_t s = 0; s != mesh.getIncidentSize(n); ++s) {
      // For each edge.
      for (std::size_t i = 0; i != Mesh::M; ++i) {
         for (std::size_t j = i + 1; j != Mesh::M + 1; ++j) {
            d = squaredDistance(mesh.getSimplexVertex(s, i),
...
maximumRelativePenetration(const IndSimpSetIncAdj<N, N, T>& mesh) {
   ...
   typedef std::tr1::tuple<std::size_t, std::size_t, Vertex> Record;
   ...
   for (std::size_t i = 0; i != components.size(); ++i) {
      ...
      for (std::size_t j = 0; j != components.size(); ++j) {
         ...
         for (std::size_t k = 0; k != boundaryIndices[j].size(); ++k) {
            ...
            vertices.push_back(components[j].getVertex(boundaryIndices[j][k]));
         }
      }
      ...
      for (std::size_t j = 0; j != penetrations.size(); ++j) {
         ...
         const std::size_t pi = std::tr1::get<0>(penetrations[j]);
         T d = euclideanDistance(vertices[pi], std::tr1::get<2>(penetrations[j]));
