...
buildDistinctPoints(PtForIter pointsBeginning, PtForIter pointsEnd,
   ...
   // A Cartesian point.
   typedef std::tr1::array<T, N> CartesianPoint;
   ...
   BBox domain;
   domain.bound(pointsBeginning, pointsEnd);
   ...
   for (; pointsBeginning != pointsEnd; ++pointsBeginning) {
      ...
      for (m = 0; m != size; ++m) {
         if (squaredDistance(p, *candidates[m]) <= minSquaredDistance) {
            break;
         }
      }
...
removeDuplicateVertices(IndSimpSet<N, M, T>* x, const T minDistance) {
   ...
   if (vertices.size() == x->getVerticesSize()) {
      // Do nothing.
      return;
   }

   assert(vertices.size() < x->getVerticesSize());
   assert(indices.size() == x->getVerticesSize());

   //
   // Update the vertices.
   //
   vertices.swap(x->getVertices());
