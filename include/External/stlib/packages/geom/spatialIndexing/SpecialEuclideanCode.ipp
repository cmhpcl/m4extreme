...
SpecialEuclideanCode(const BBox& domain, const double spacing) :
   ...
   _bitsPerTranslation(),
   _centroids() {
   LOKI_STATIC_CHECK(_SubdivisionLevels > 0, BadNumberOfSubdivisions);
   ...
   assert(2 * BitsPerAxis + sum(_bitsPerTranslation) <= 
          std::size_t(std::numeric_limits<_Key>::digits));
   // From the number of bits, determine the cell extents.
   std::tr1::array<std::size_t, D> extents;
   for (std::size_t i = 0; i != D; ++i) {
      extents[i] = 1 << _bitsPerTranslation[i];
   }
   // Determine the lower bound.
   _lower = 0.5 * (domain.lower + domain.upper) - (0.5 * spacing) * extents;
...
computeCentroids() {
   ...
   typedef std::tr1::array<Point, D> Triangle;

   std::tr1::array<Point, 6> axes = {{
         {{-1, 0, 0}}, // -x
         {{1, 0, 0}},  // +x
         {{0, -1, 0}}, // -y
         {{0, 1, 0}},  // +y
   ...
   std::tr1::array<std::vector<Triangle>, _SubdivisionLevels+1> corners;
   ...
   std::tr1::array<Point, 3> midpoints;
...
computeAdjacencies() {
   // Start by computing the corners of the triangles.
   std::tr1::array<std::vector<AdjacentList>, _SubdivisionLevels+1> adj;
   
   adj[0].resize(8);
   // 1 2 4
   adj[0][0][0] = 1; adj[0][0][1] = 2; adj[0][0][2] = 4;
   // 0 3 5
   adj[0][1][0] = 0; adj[0][1][1] = 3; adj[0][1][2] = 5;
   // 3 0 6
   adj[0][2][0] = 3; adj[0][2][1] = 0; adj[0][2][2] = 6;
   // 2 1 7
   adj[0][3][0] = 2; adj[0][3][1] = 1; adj[0][3][2] = 7;
   // 5 6 0
   adj[0][4][0] = 5; adj[0][4][1] = 6; adj[0][4][2] = 0;
   // 4 7 1
   adj[0][5][0] = 4; adj[0][5][1] = 7; adj[0][5][2] = 1;
   // 7 4 2
   adj[0][6][0] = 7; adj[0][6][1] = 4; adj[0][6][2] = 2;
   // 6 5 3
   adj[0][7][0] = 6; adj[0][7][1] = 5; adj[0][7][2] = 3;
...
encode(const Quaternion& q, const Point& translation) const {
   ...
   xq = q * xq * conj(q);
   const Point x = {{xq.R_component_2(), xq.R_component_3(),
                     xq.R_component_4()}};
   Quaternion yq(0, 0, 1, 0);
   yq = q * yq * conj(q);
   const Point y = {{yq.R_component_2(), yq.R_component_3(),
                     yq.R_component_4()}};
...
decode(Key key, Point* x, Point* y, Point* translation) const {
   ...
   *y -= dot(*x, *y) * *x;
#ifdef DEBUG_stlib
   assert(numerical::areEqual(dot(*x, *y), 0., 10.));
#endif
...
encode(const Point& axis) const {
   ...
   assert(numerical::areEqual(squaredMagnitude(axis), 1., 100.));
#endif
   Key key = 0;
   ...
   for (std::size_t i = 0; i != _SubdivisionLevels; ++i) {
      ...
      for (std::size_t j = 0; j != 4; ++j) {
         double d = squaredDistance(axis, _centroids[i][4*key+j]);
         if (d < minDistance) {
            minDistance = d;
            closest = j;
         }
...
neighbors(Key key, std::vector<Key>* result) const {
   ...
   std::tr1::array<std::size_t, D> translation;
   for (std::size_t i = D; i-- != 0; ) {
      const Key translationMask = ((1 << _bitsPerTranslation[i]) - 1);
   ...
   const std::size_t BitsPerTranslation = sum(_bitsPerTranslation);
   result->clear();
...
void
SpecialEuclideanCode<3, _SubdivisionLevels, _Key>::
reportNeighbors(const std::tr1::array<std::size_t, D>& cell,
                std::vector<Key>* neighbors) const {
   // Compute the closed lower and open upper bound.
   std::tr1::array<std::size_t, D> lower, upper;
   for (std::size_t i = 0; i != D; ++i) {
      if (cell[i] != 0) {
         lower[i] = cell[i] - 1;
      }
   ...
   neighbors->clear();
   std::tr1::array<std::size_t, D> index;
