...
template < std::size_t _Dimension, std::size_t _MaximumLevel,
         ...
         template<std::size_t, std::size_t> class _Key = SpatialIndex >
class
   OrthtreeMap :
   public std::map<_Key<_Dimension, _MaximumLevel>, _Element> {
   //
   // Private types.
   //
private:

   typedef std::map<_Key<_Dimension, _MaximumLevel>, _Element> Base;
...
public:
   ...
   static const bool AutomaticBalancing = _AutomaticBalancing;
   //! The number of orthants.
   static const std::size_t NumberOfOrthants = Key::NumberOfOrthants;
...
private:
   ...
   Point _lowerCorner;
   std::tr1::array < Point, MaximumLevel + 1 > _extents;
   Split _split;
   Merge _merge;
   Refine _refine;
   Coarsen _coarsen;
...
private:
   ...
   evaluate(const typename _Function::result_type& (_Function::*/*dummy*/)() const,
            _Function f, const_iterator i) const {
      return f();
   }
   ...
   //! result_type (const std::tr1::array<const Element*,NumberOfOrthants>&) const
   template<typename _Function>
   typename _Function::result_type
   evaluate(typename _Function::result_type(_Function::*/*dummy*/)
            (const std::tr1::array<const Element*, NumberOfOrthants>&) const,
            _Function f, const_iterator i) const {
      std::tr1::array<const Element*, NumberOfOrthants> elements;
...
private:
   ...
   evaluateSplit(Element(_Function::*/*dummy*/)(const Point&) const,
                 _Function f, const Element& parent, const int n, const Key& key,
                 Element* element) {
...
private:
   ...
   evaluateMerge(void (_Function::*/*dummy*/)() const,
                 _Function f, const iterator firstChild, const Key& parentKey,
                 Element* parentElement) {
   }
   ...
   //! Element operator()(const std::tr1::array<const Element*,NumberOfOrthants>&) const
   template<typename _Function>
   void
   evaluateMerge(Element(_Function::*/*dummy*/)
                 (const std::tr1::array<const Element*, NumberOfOrthants>&) const,
   ...
   //! const Element& operator()(const std::tr1::array<const Element*,NumberOfOrthants>&) const
   template<typename _Function>
   void
   evaluateMerge(const Element& (_Function::*/*dummy*/)
                 (const std::tr1::array<const Element*, NumberOfOrthants>&) const,
   ...
   //! void operator()(const std::tr1::array<const Element*,NumberOfOrthants>&, Element*) const
   template<typename _Function>
   void
   evaluateMerge(void (_Function::*/*dummy*/)
                 (const std::tr1::array<const Element*, NumberOfOrthants>&,
                  Element*) const,
                 _Function f, iterator firstChild, const Key& parentKey,
                 Element* parentElement) {
      // Make an array of const pointers to the elements.
...
(const OrthtreeMap < _Dimension, _MaximumLevel, _Element, _AutomaticBalancing,
 ...
 _Element initial) {
   typedef OrthtreeMap < _Dimension, _MaximumLevel, _Element, _AutomaticBalancing,
           _Split, _Merge, _Refine, _Coarsen, _Action, _Key >
           OrthtreeMap;
