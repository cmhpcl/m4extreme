...
private:
   ...
   typedef array::MultiArray<Cell, N> DenseArray;
...
public:
   ...
   */
   CellArray(const Point& delta, const BBox& domain) :
      Base(delta, domain),
   ...
   */
   template<class ForwardIterator>
   CellArray(const std::size_t numberOfCells, ForwardIterator first,
   ...
   */
   ...
   insert(const Record record) {
      Cell& b = (*this)(_multiKeyAccessor(record));
      b.push_back(record);
      incrementSize();
   }
