...
// radius member of Ball<3, _T>.  Only at the end is a sqrt computed.
template<typename _T>
MinBall3<_T>::
MinBall3(int iQuantity, const Point* akPoint,
         Ball<3, _T>& rkMinimal, _T epsilon) :
   _epsilon(epsilon) {
   m_aoUpdate[0] = 0;
      ...
      while (i < iQuantity) {
         if (!kSupp.Contains(i,apkPerm,_epsilon)) {
            if (!Contains(*apkPerm[i],rkMinimal,distDiff)) {
               ...
               Ball<3, _T> kSph =(this->*oUpdate)(i,apkPerm,kSupp);
...
Contains(const Point& rkP,
         const Ball<3, _T>& rkS, _T& rdistDiff) {
...
template<typename _T>
Ball<3, _T> MinBall3<_T>::ExactSphere1(const Point& rkP) {
   Ball<3, _T> kMinimal;
   kMinimal.Center = rkP;
   kMinimal.Radius = 0.;
   return kMinimal;
...
template<typename _T>
Ball<3, _T> MinBall3<_T>::ExactSphere2(const Point& rkP0,
      const Point& rkP1) {
   Ball<3, _T> kMinimal;
...
MinBall3<_T>::UpdateSupport1(int i,
   ...
   Ball<3, _T> kMinimal = ExactSphere2(rkP0,rkP1);
   rkSupp.Quantity = 2;
   rkSupp.Index[1] = i;
