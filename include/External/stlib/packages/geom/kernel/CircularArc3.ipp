...
make(const Point& center, const Point& source, const Point& target) {
   ...
   // The circle radius.
   _circle.radius = euclideanDistance(center, source);
   // CONTINUE: This check is ad-hoc.
   const Number r = euclideanDistance(center, target);
...
isValid() const {
   ...
   if (std::abs(magnitude(_axis0) - 1.0) >
         10.0 * std::numeric_limits<T>::epsilon()) {
      return false;
   }
   if (std::abs(magnitude(_axis1) - 1.0) >
         10.0 * std::numeric_limits<T>::epsilon()) {
      return false;
   }
...
computeClosestPoint(const CircularArc3<T>& circularArc,
   ...
   const T x0 = dot(circularArc.getFirstAxis(), p);
   const T x1 = dot(circularArc.getSecondAxis(), p);
   // Compute the angle.
   ...
   if (squaredDistance(x, source) < squaredDistance(x, target)) {
      *closestPoint = source;
   }
   else {
      *closestPoint = target;
...
computeClosestPoint(const Circle3<T>& circle,
   ...
   // CONTINUE REMOVE
#if 0
   std::cout << "compute closest point: circle and circular arc.\n"
             << "circle = " << circle << "\n"
             << "circular arc = " << circularArc << "\n"
             << "numberOfSteps = " << numberOfSteps << "\n"
             << "distance = " << euclideanDistance(pointOnArc, *closestPoint)
             << "\n"
             << "closest point = " << *closestPoint << "\n"
             << "point on arc = " << pointOnArc << "\n"
             << "error = " << (euclideanDistance(pointOnCircle, *closestPoint) *
                               circle.getRadius()) << "\n\n";
#endif
}
