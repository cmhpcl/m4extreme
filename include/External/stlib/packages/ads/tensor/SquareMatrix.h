...
inline
void
computeProduct(const SquareMatrix<N, T>& m, const std::tr1::array<T, N>& v,
               std::tr1::array<T, N>* x) {
   LOKI_STATIC_CHECK(N > 3, N_greater_than_3);
   *x = 0;
   typename SquareMatrix<N, T>::const_iterator mi = x->begin();
   typename std::tr1::array<T, N>::const_iterator vi;
   const typename std::tr1::array<T, N>::const_iterator vi_end = v.end();
   // Loop over the rows.
...
inline
void
computeProduct(const SquareMatrix<1, T>& m, const std::tr1::array<T, 1>& v,
               std::tr1::array<T, 1>* x) {
   (*x)[0] = m[0] * v[0];
}
...
inline
void
computeProduct(const SquareMatrix<2, T>& m, const std::tr1::array<T, 2>& v,
               std::tr1::array<T, 2>* x) {
   (*x)[0] = m[0] * v[0] + m[1] * v[1];
   (*x)[1] = m[2] * v[0] + m[3] * v[1];
}
...
inline
void
computeProduct(const SquareMatrix<3, T>& m, const std::tr1::array<T, 3>& v,
               std::tr1::array<T, 3>* x) {
