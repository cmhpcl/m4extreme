      ...
      for (MultiIndexRangeIterator i = MultiIndexRangeIterator::begin(getRanges());
         ...
         if (gd == HugePoint) {
            ...
            if (isClosestPointBeingComputed()) {
               ...
               cp = getClosestPoint()(*i);
               if (magnitude(cross(position - cp, gd)) >
...
floodFill(const Number farAway) {
   ...
   bool result = false;
   int sign = 0;
   typename array::MultiArray<Number, 3>::const_iterator iter =
      getDistance().begin();
   const typename array::MultiArray<Number, 3>::const_iterator iterEnd =
      getDistance().end();
