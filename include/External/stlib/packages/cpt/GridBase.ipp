...
computeClosestPointTransform(const Lattice& lattice,
   ...
   Range indexRange = array::overlap(indexRangeInLattice, getRanges());
...
floodFillUnsigned(const Number farAway) {
   ...
   //
   typename array::MultiArray<Number, N>::iterator i = getDistance().begin(),
...
copyToDistanceExternal() const {
   if (_distanceExternal) {
      ...
      for (typename array::MultiArray<Number, N>::const_iterator
...
copyToExternal() const {
   ...
   if (_gradientOfDistanceExternal) {
      ...
      for (typename array::MultiArray<Point, N>::const_iterator
   ...
   if (_closestFaceExternal) {
      ...
      for (typename array::MultiArray<std::size_t, N>::const_iterator
...
put(std::ostream& out) const {
   ...
   std::copy(_distance.begin(), _distance.end(),
             std::ostream_iterator<Number>(out, "\n"));
   std::copy(_gradientOfDistance.begin(), _gradientOfDistance.end(),
             std::ostream_iterator<Point>(out, "\n"));
   std::copy(_closestPoint.begin(), _closestPoint.end(),
             std::ostream_iterator<Point>(out, "\n"));
   std::copy(_closestFace.begin(), _closestFace.end(),
...
countKnownDistances(const Number maximumDistance) const {
   ...
   typename array::MultiArray<Number, N>::const_iterator
   ptr = _distance.begin();
   const typename array::MultiArray<Number, N>::const_iterator
   end = _distance.end();
   for (; ptr != end; ++ptr) {
