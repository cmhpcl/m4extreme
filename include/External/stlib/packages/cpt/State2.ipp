...
public:
   ...
   */
   ...
   determinePointsInside(const BBox& domain,
                         array::MultiArrayRef<bool, 2>* areInside) {
      array::MultiArray<Number, 2> distance(areInside->extents(),
                                            areInside->bases());
   ...
   */
   ...
   determinePointsOutside(const BBox& domain,
      ...
      for (typename array::MultiArrayRef<bool, 2>::iterator
            i = areOutside->begin(); i != areOutside->end(); ++i) {
         *i = ! *i;
      }
      for (typename array::MultiArrayRef<Number, 2>::iterator
...
decimateVertices(std::vector<Point>* decimatedVertices) {
   ...
   const std::size_t verticesSize = _brep.getVerticesSize();
   if (_decimationFactor >= 1) {
      for (std::size_t i = 0; i < verticesSize; i += _decimationFactor) {
         decimatedVertices->push_back(_brep.getVertex(i));
      }
   }
}
...
determinePointsInside(const BBox& domain,
   ...
   setParameters(domain, 1.01 * max(_lattice.getDelta()));
   // Clear any old grids.
   clearGrids();
   // Add the distance grid.  Use empty grids for the rest.
   {
      array::MultiArray<Point, 2> gradientOfDistance;
      array::MultiArray<Point, 2> closestPoint;
      array::MultiArray<std::size_t, 2> closestFace;
   ...
   if (counts.second == 0) {
      ...
      if (signedDistance(domain.getLowerCorner()) <= 0) {
   ...
   typename array::MultiArray<bool, 2>::iterator i = areInside->begin();
   const typename array::MultiArray<bool, 2>::iterator iEnd = areInside->end();
   typename array::MultiArray<Number, 2>::const_iterator j = distance->begin();
   for (; i != iEnd; ++i, ++j) {
...
determinePointsInside(const Point& lower,
                      ...
                      bool* areInside,
                      Number* distance) {
   array::MultiArrayRef<bool, 2> areInsideArray(areInside, extents);
   array::MultiArrayRef<Number, 2> distanceArray(distance, extents);
...
determinePointsOutside(const Point& lower,
                       ...
                       bool* areOutside,
                       Number* distance) {
   array::MultiArrayRef<bool, 2> areOutsideArray(areOutside, extents);
   array::MultiArrayRef<Number, 2> distanceArray(distance, extents);
