   ...
   if (isGradientOfDistanceBeingComputed()) {
      ...
      for (MultiIndexRangeIterator i = MultiIndexRangeIterator::begin(getRanges());
         ...
         if (gd == HugePoint) {
            ...
            if (std::abs(magnitude(gd) - 1) > Eps10) {
               std::cerr << "In Grid<2,T>::isValid():\n"
                         << "    Magnitude of gradient, " << magnitude(gd)
                         << ", is not unity.\n"
                         << "    (i,j) = " << *i << '\n';
               result = false;
               if (++numberOfErrors >= maximumReportedErrors) {
            ...
            if (isClosestPointBeingComputed()) {
               ...
               cp = getClosestPoint()(*i);
               if (std::abs(discriminant(position - cp, gd)) >
...
floodFill(const Number farAway) {
   ...
   bool result = false;
   int sign = 0;
   typename array::MultiArray<Number, 2>::const_iterator iter =
      getDistance().begin();
   const typename array::MultiArray<Number, 2>::const_iterator iter_end =
      getDistance().end();
