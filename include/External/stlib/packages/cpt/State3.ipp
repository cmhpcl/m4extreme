...
public:
   ...
   */
   ...
   determinePointsInside(const BBox& domain,
                         array::MultiArrayRef<bool, 3>* areInside,
                         array::MultiArrayRef<Number, 3>* distance);
   ...
   */
   ...
   determinePointsInside(const BBox& domain,
                         array::MultiArrayRef<bool, 3>* areInside) {
      array::MultiArray<Number, 3> distance(areInside->extents(),
                                            areInside->bases());
   ...
   */
   ...
   determinePointsOutside(const BBox& domain,
                          array::MultiArrayRef<bool, 3>* areOutside,
                          array::MultiArrayRef<Number, 3>* distance) {
      ...
      for (typename array::MultiArrayRef<bool, 3>::iterator
            i = areOutside->begin(); i != areOutside->end(); ++i) {
         *i = ! *i;
      }
      for (typename array::MultiArrayRef<Number, 3>::iterator
...
determinePointsInside(const BBox& domain,
   ...
   {
      array::MultiArray<Point, 3> gradientOfDistance;
      array::MultiArray<Point, 3> closestPoint;
      array::MultiArray<std::size_t, 3> closestFace;
   ...
   typename array::MultiArray<bool, 3>::iterator i = areInside->begin();
   const typename array::MultiArray<bool, 3>::iterator iEnd = areInside->end();
   typename array::MultiArray<Number, 3>::const_iterator j = distance->begin();
   for (; i != iEnd; ++i, ++j) {
...
determinePointsInside(const Point& lower,
                      ...
                      bool* areInside,
                      Number* distance) {
   array::MultiArrayRef<bool, 3> areInsideArray(areInside, extents);
   array::MultiArrayRef<Number, 3> distanceArray(distance, extents);
...
determinePointsOutside(const Point& lower,
                       ...
                       bool* areOutside,
                       Number* distance) {
   array::MultiArrayRef<bool, 3> areOutsideArray(areOutside, extents);
   array::MultiArrayRef<Number, 3> distanceArray(distance, extents);
