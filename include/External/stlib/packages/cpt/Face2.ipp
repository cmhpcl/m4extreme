...
public:
   ...
   //! A Cartesian point.
   typedef std::tr1::array<T, 2> Point;
...
Face(const Point& source,
   ...
   _source(source),
   _target(target),
   _length(euclideanDistance(_source, _target)),
   _tangent(normal),
   _normal(normal),
   _index(index),
...
computeDistanceUnsigned(const Point& p) const {
   ...
   if (proj < 0.0) {  // If the point is closest to the source end.
      dist = euclideanDistance(p, getSource());
   }
   else if (proj > getLength()) { // If the point is closest to the target end.
      dist = euclideanDistance(p, getTarget());
   }
   else { // If the point is closest to the interior of the edge.
      // Unsigned distance to the line defined by this edge.
      dist = std::abs(dot(p - getSource(), getNormal()));
   }
...
buildCharacteristicPolygon(Polygon* polygon, const Face& prev,
   ...
   const Number rightDiscriminant =
      discriminant(prev.getTangent(), getTangent());
   const Number leftDiscriminant =
      discriminant(getTangent(), next.getTangent());
   ...
   if (rightDiscriminant < -discriminantThreshhold) {
      Number x = dot(prev.getSource() - tgt, getTangent());
      Number d = dot(prev.getSource() - tgt, getNormal());
#ifdef DEBUG_stlib
      assert(d > 0);
#endif
   ...
   if (leftDiscriminant < -discriminantThreshhold) {
      Number x = dot(next.getTarget() - src, getTangent());
      Number d = dot(next.getTarget() - src, getNormal());
#ifdef DEBUG_stlib
      assert(d > 0);
#endif
      Number y = (x * x + d * d) / (2 * d);
