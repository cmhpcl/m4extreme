...
makeEquidistantLine(geom::Line_2<T>* line,
                    const std::tr1::array<T, 2>& p,
                    const std::tr1::array<T, 2>& q) {
   assert(p != q);
   std::tr1::array<T, 2> t = T(0.5) * (p - q);
   std::tr1::array<T, 2> n = t;
...
clip(geom::ScanConversionPolygon<std::ptrdiff_t, T>* poly,
     ...
     const std::vector< std::tr1::array<T, 2> >& clipPoints) {
   geom::Line_2<T> line;
   std::tr1::array<T, 2> displacement;
   for (typename std::vector< std::tr1::array<T, 2> >::const_iterator
      ...
      if (vertex.getLocation() != *i &&
            dot(displacement, vertex.getRightNormal()) > 0 &&
            dot(displacement, vertex.getLeftNormal()) > 0) {
...
template<typename T>
bool
computeBestClipPoint(const std::tr1::array<T, 2>& point,
                     const std::tr1::array<T, 2>& normal,
                     const std::vector< std::tr1::array<T, 2> >& clipPoints,
                     std::tr1::array<T, 2>* bestPoint) {
   T minimumValue = std::numeric_limits<T>::max();
   T val, den;
   std::tr1::array<T, 2> vec;
...
oneClip(geom::ScanConversionPolygon<std::ptrdiff_t, T>* poly,
        const Vertex<2, T>& vertex,
        const std::vector<std::tr1::array<T, 2> >& clipPoints) {
   std::tr1::array<T, 2> normal = vertex.getRightNormal() +
...
bool
makeEquidistantLine(geom::Line_2<T>* line,
                    const std::tr1::array<T, 2>& source,
                    const std::tr1::array<T, 2>& target,
                    const std::tr1::array<T, 2>& tangent,
                    const std::tr1::array<T, 2>& normal,
                    const std::tr1::array<T, 2>& point) {
   const T d = dot(normal, point - source);
   // Return false if the point is not above the segment.
   if (d <= 0) {
      return false;
   }

   T x = dot(point - target, tangent);
   const std::tr1::array<T, 2> p1 = target + ((x * x + d * d) / (2 * d)) * normal;
   x = dot(point - source, tangent);
   const std::tr1::array<T, 2> p2 = source + ((x * x + d * d) / (2 * d)) * normal;
...
clip(geom::ScanConversionPolygon<std::ptrdiff_t, T>* poly,
     ...
     const std::vector< std::tr1::array<T, 2> >& clipPoints) {
   geom::Line_2<T> line;
   typename std::vector< std::tr1::array<T, 2> >::const_iterator
   i = clipPoints.begin();
   const typename std::vector< std::tr1::array<T, 2> >::const_iterator
...
oneClip(geom::ScanConversionPolygon<std::ptrdiff_t, T>* poly,
        ...
        const std::vector< std::tr1::array<T, 2> >& clipPoints) {
   std::tr1::array<T, 2> bestPoint;
   const std::tr1::array<T, 2> mid_point = T(0.5) *
   ...
   */
   ...
   getVertexUnsigned(const std::size_t index, VertexDistance* vert) const {
      ...
      if (right != Unknown && left != Unknown) {
         ...
         vert->make(getVertex(index), ext::filled_array<Point>(0.0),
   ...
   // Enlarge the bounding box by the maximumDistance.
   void
   getFaceBBox(const std::size_t index, const Number maximumDistance, BBox* box) const {
      box->bound(getSimplexVertex(index, 0),
                 getSimplexVertex(index, 1));
      box->setLowerCorner(box->lower - maximumDistance);
      box->setUpperCorner(box->upper + maximumDistance);
   }
};
