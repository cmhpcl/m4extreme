...
(const std::size_t numberOfSpecies,
 ...
 array::StaticArrayOfArrays<std::size_t>* influence,
 const bool includeSelf) {
   typedef array::SparseVector<std::size_t> SparseVector;
   ...
   //
   array::StaticArrayOfArrays<std::size_t> reactantsToReactions;
   if (numberOfSpecies != 0) {
      ...
      for (ForwardIterator reaction = reactionsBeginning;
         ...
         const SparseVector& reactants = reaction->getReactants();
         // For each reactant.
      ...
      // Allocate memory.
      reactantsToReactions.rebuild(sizes.begin(), sizes.end());
      ...
      for (ForwardIterator reaction = reactionsBeginning;
         ...
         // For each reactant.
         for (const_iterator i = reactants.begin(); i != reactants.end(); ++i) {
            reactantsToReactions(i->first, sizes[i->first]) = reactionIndex;
            ++sizes[i->first];
         }
      }
   }
   ...
   for (ForwardIterator reaction = reactionsBeginning; reaction != reactionsEnd;
      ...
      for (std::set<std::size_t>::const_iterator species =
         ...
         for (std::size_t n = 0; n != reactantsToReactions.size(*species); ++n) {
            const std::size_t r = reactantsToReactions(*species, n);
