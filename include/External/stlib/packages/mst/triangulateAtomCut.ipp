...
clipWithCuts(const geom::Ball<T, 3>& atom, const geom::Ball<T, 3>& clippingAtom,
   ...
   std::size_t vertexIndex = mesh->getVerticesSize();
   ...
   std::vector<std::tr1::array<int, M+1> >
      newVertexIndices(mesh->getSimplicesSize(),
                       ext::filled_array<std::tr1::array<int, M+1> >(-1));
   ...
   for (simplexIndex = 0; simplexIndex != mesh->getSimplicesSize();
      ...
      std::tr1::array<int, M + 1> simplexSignOfDistance;
      ...
      std::tr1::array<int, M + 1> sd(simplexSignOfDistance);
      ...
      // If three vertices are on the clipping surface.
         ...
         if (clippingAtom.radius -
             euclideanDistance(clippingAtom.center, centroid) < 0) {
            // Keep the triangle.
            indexedSimplices.push_back(s);
         }
      }
...
clipWithCuts(const geom::Ball<T, 3>& atom, const geom::Ball<T, 3>& clippingAtom,
   ...
   for (std::size_t i = 0; i != distance.size(); ++i) {
      distance[i] = clippingAtom.radius
         - euclideanDistance(clippingAtom.center, mesh->getVertex(i));
   }
