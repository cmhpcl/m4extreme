...
operator()(const Number* nodeCoordinates,
   ...
   if (_comm.Get_rank() == 0) {
      for (std::size_t i = 0; i != penetrations.size(); ++i) {
         std::cout << std::tr1::get<0>(penetrations[i]) << ' '
                   << std::tr1::get<1>(penetrations[i]) << ' '
                   << std::tr1::get<2>(penetrations[i]) << '\n';
      }
   }
#endif
   std::vector<std::tr1::tuple<std::size_t, Point> > allForces;
...
void
ProxyBallContactConcurrent<N, _Identifier>::
exchangeForces(const std::vector<std::tr1::tuple<std::size_t, Point> >&
   ...
   for (std::size_t i = 0; i != allForces.size(); ++i) {
      const std::size_t index = std::tr1::get<0>(allForces[i]);
      // If this is a force on a local proxy ball.
      if (index < _numberOfLocalProxyBalls) {
         *elementForces++ = allForces[i];
#if 0
         if (_comm.Get_rank() == 0) {
            std::cout << "local " << std::tr1::get<0>(allForces[i]) << ' '
                      << std::tr1::get<1>(allForces[i]) << '\n';
         }
#endif
      }
      else {
         ...
         std::tr1::get<0>(force) = _proxyBalls[index].elementIndex;
         // Record the force.
         forcesToSend[_proxyBalls[index].processor].push_back(force);
#if 0
         if (_comm.Get_rank() == 0) {
            std::cout << "ghost " << std::tr1::get<0>(force) << ' '
                      << std::tr1::get<1>(force) << '\n';
         }
#endif
      }
