...
operator()(const Number* nodeCoordinates,
   ...
   std::vector<std::tr1::tuple<std::size_t, std::size_t, Point> > penetrations;
...
operator<<(std::ostream& out, const ProxyBallContact<N, _Identifier>& x) {
   typedef typename ProxyBallContact<N, _Identifier>::HashTable::value_type Value;
   ...
   std::copy(x._springAndDampingConstants.begin(),
             x._springAndDampingConstants.end(),
...
void
ProxyBallContact<N, _Identifier>::
computeForces(const std::vector < std::tr1::tuple < std::size_t, std::size_t,
              Point > > penetrations,
              _ForceOutputIterator elementForces,
              _CountOutputIterator interactionCounts,
              _RelPenOutputIterator relativePenetrations) {
   ...
   std::tr1::array<ProxyBallConstIterator, 2> pb;
   // The identifiers of the source and target proxy balls.
   std::tr1::array<std::size_t, 2> id;
   ...
   for (std::size_t n = 0; n != penetrations.size(); ++n) {
      ...
      const Number displacement = magnitude(direction);
      normalize(&direction);
      ...
      *relativePenetrations++ = displacement / sumOfRadii;
      ...
      if (springConstantIterator == savedSpringConstants.end()) {
         ...
         for (std::size_t i = 0; i != 2; ++i) {
            ...
            nv[i] = dot(pb[i]->velocity, direction);
         }
...
computeContact(_OutputIterator contacts) {
   ...
   typedef geom::CellArrayStatic < N, Record, Point, Number,
           GetProxyBallCenter<Record, Point> > Orq;
   ...
   BBox bb;
   Point p;
   std::tr1::tuple<std::size_t, std::size_t, Point> penetration;
      ...
      for (std::size_t other = 0; other != _numberOfComponents; ++other) {
         ...
         for (std::size_t k = 0; k != _components[other].size(); ++k) {
            ...
            for (std::size_t n = 0; n != potential.size(); ++n) {
               ...
               const Number sd = squaredDistance(source.center, target.center);
               ...
               if (sd < radiiSum * radiiSum) {
                  ...
                  std::tr1::get<0>(penetration) = sourceIndex;
                  std::tr1::get<1>(penetration) = targetIndex;
...
computeStableTimeStep(const std::size_t* components) const {
   ...
   for (std::size_t i = 0; i != _proxyBalls.size(); ++i) {
      ...
      const Number speed = magnitude(_proxyBalls[i].velocity);
...
(const std::tr1::array < std::tr1::array<T, N>, N + 1 > & simplex,
