...
inline
T
computeMidpointRadius(const std::tr1::array < std::tr1::array<T, N>, N + 1 > & simplex,
                      const std::tr1::array<T, N>& centroid) {
   std::tr1::array<T, N> x;
   T squaredRadius = 0;
...
inline
T
computeMidpointRadius(const std::tr1::array<std::tr1::array<T, 1>, 2>& simplex,
                      const std::tr1::array<T, 1>& centroid) {
...
operator()(const std::vector<std::tr1::array<T, N> >& vertices,
           const std::vector<std::size_t>& vertexIdentifiers,
           const std::vector<std::tr1::array<T, N> >& velocities,
           const std::vector < std::tr1::array < std::size_t, N + 1 > > &
           identifierSimplices,
   ...
   typedef std::tr1::array<Number, N> Point;
   ...
   // Make a mesh.
   Mesh mesh(vertices, vertexIdentifiers, identifierSimplices);

   // Get the penetrations in terms of the simplex indices.
   std::vector<std::tr1::tuple<std::size_t, std::size_t, Point, Number> >
   penetrations;
   ...
   std::vector<Point> forces(mesh.getSimplicesSize(),
                             ext::filled_array<Point>(0.));
   // The number of contact interactions for each element.
   std::vector<std::size_t> interactions(mesh.getSimplicesSize(), 0);
   Point v, force;
   Number forceMagnitude;
   // The indices of the source and target simplices. (Simplex Index)
   std::tr1::array<std::size_t, 2> si;
   // The masses of the balls.
   std::tr1::array<Number, 2> bm;
   // The normal component of the velocity.
   std::tr1::array<Number, 2> nv;
   Number springConstant, dampingConstant;
   // For each contact.
   for (std::size_t n = 0; n != penetrations.size(); ++n) {
      // The source simplex.
      si[0] = std::tr1::get<0>(penetrations[n]);
      ++interactions[si[0]];
      // The target simplex.
      si[1] = std::tr1::get<1>(penetrations[n]);
      ++interactions[si[1]];
      // The spring displacement.
      const Number displacement = magnitude(std::tr1::get<2>(penetrations[n]));
      Point direction = std::tr1::get<2>(penetrations[n]);
      normalize(&direction);

      // The sum of the ball radii.
      const Number sumOfRadii = std::tr1::get<3>(penetrations[n]);
      ...
      if (springConstantIterator == savedSpringConstants.end()) {
         ...
         for (std::size_t i = 0; i != 2; ++i) {
            ...
            for (std::size_t m = 0; m != Mesh::M + 1; ++m) {
               v += velocities[mesh.getIndexedSimplex(si[i])[m]];
            }
            v *= 1. / (Mesh::M + 1);
            // Component along penetration vector.
            nv[i] = dot(v, direction);
         }
   ...
   std::tr1::tuple<std::size_t, Point> report;
   for (std::size_t i = 0; i != forces.size(); ++i) {
      if (forces[i] != zero) {
         std::tr1::get<0>(report) = i;
         std::tr1::get<1>(report) = forces[i];
...
inline
T
pinballContact(const std::vector<std::tr1::array<T, N> >& vertices,
               const std::vector<std::size_t>& vertexIdentifiers,
               const std::vector<std::tr1::array<T, N> >& velocities,
               const std::vector < std::tr1::array < std::size_t, N + 1 > > & simplices,
               const T maximumRelativePenetration,
               OutputIterator contacts) {
   typedef geom::IndSimpSetIncAdj<N, N> Mesh;
   // Make a mesh.
   Mesh mesh(vertices, vertexIdentifiers, simplices);
   // Get the penetrations in terms of the simplex indices.
   return pinballContact(mesh, velocities, maximumRelativePenetration, contacts);
}
...
pinballContact(const geom::IndSimpSetIncAdj<N, N, T>& mesh,
   ...
   typedef geom::CellArrayStatic<N, Record> Orq;
   ...
   std::vector<Number> radii(mesh.getSimplicesSize());
   {
      Simplex s;
      //std::tr1::array<Number, Mesh::M+1> r;
   ...
   BBox bb;
   Point p;
   std::tr1::tuple<std::size_t, std::size_t, Point, Number> penetration;
   ...
   for (std::size_t component = 0; component != numComponents; ++component) {
      ...
      for (std::size_t i = 0; i != centroids.size(); ++i) {
         ...
         for (std::size_t n = 0; n != potential.size(); ++n) {
            ...
            const Number sd = squaredDistance(centroids[i], centroids[j]);
            ...
            if (sd < radiiSum * radiiSum) {
               ...
               std::tr1::get<0>(penetration) = i;
               std::tr1::get<1>(penetration) = j;
               std::tr1::get<2>(penetration) = p;
               std::tr1::get<3>(penetration) = radiiSum;
   ...
   for (std::size_t i = 0; i != mesh.getSimplicesSize(); ++i) {
      ...
      std::fill(v.begin(), v.end(), 0);
      for (std::size_t m = 0; m != Mesh::M + 1; ++m) {
         v += velocities[mesh.getIndexedSimplex(i)[m]];
      }
      v *= 1. / (Mesh::M + 1);
      const Number speed = magnitude(v);
...
inline
T
computeRadiusFromContent(const T content, Loki::Int2Type<1> /*Dimension*/) {
   return 0.5 * content;
}
...
inline
T
computeRadiusFromContent(const T content, Loki::Int2Type<2> /*Dimension*/) {
...
inline
T
computeRadiusFromContent(const T content, Loki::Int2Type<3> /*Dimension*/) {
...
(const std::tr1::array<std::tr1::array<T, 1>, 2>& simplex) {
   // c = 2 r
   // r = c / 2
...
T
computeRadiusFromSimplex
(const std::tr1::array<std::tr1::array<T, 2>, 3>& simplex) {
   // c = pi r^2
   // r = (c / pi)^(1/2)
...
T
computeRadiusFromSimplex
(const std::tr1::array<std::tr1::array<T, 3>, 4>& simplex) {
   // c = (4/3) pi r^3
   // r = (3 c / (4 pi))^(1/3)
