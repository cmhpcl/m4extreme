...
#include "../geom/mesh/iss/topology.h"
#include "../geom/mesh/simplex/geometry.h"
...
/*!
  The forces are reported as a std::tr1::tuple of
  (simplexIndex, force). The force is represented with a std::tr1::array<T,N>.
*/
...
public:
   ...
   typedef std::tr1::unordered_map < std::tr1::array<std::size_t, 2>,
...
operator<<(std::ostream& out, const PinballRestoringForces<N, T>& x) {
   typedef typename PinballRestoringForces<N, T>::HashTable::value_type Value;
   out << x.getSpringAndDampingConstants().size() << '\n';
   std::copy(x.getSpringAndDampingConstants().begin(), x.getSpringAndDampingConstants().end(),
...
/*!
  ...
  The contacts are reported as a std::tr1::tuple of
  ...
  The penetration vector is represented with a std::tr1::array<T,N>. The final
