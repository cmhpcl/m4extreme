...
public:
   ...
   */
   size_type
   size() const {
      return _size(Loki::Int2Type<Traits::HasGhost>());
   }
   ...
   */
   bool
   empty() const {
      return _empty(Loki::Int2Type<Traits::HasGhost>());
   }

   //! Return true if it is a ghost node.
   bool
   isGhost(const const_iterator node) const {
      return _isGhost(node, Loki::Int2Type<Traits::HasGhost>());
   }
...
private:
   ...
   size_type
   _size(Loki::Int2Type<false> /*HasGhost*/) const {
      return _nodes.size();
   }

   //! Get the number of non-ghost nodes.
   size_type
   _size(Loki::Int2Type<true> /*HasGhost*/) const {
      size_type size = 0;
   ...
   //! Return true if the tree is empty.
   bool
   _empty(Loki::Int2Type<false> /*HasGhost*/) const {
      return _nodes.empty();
   }

   //! Return true if there are no non-ghost nodes.
   bool
   _empty(Loki::Int2Type<true> /*HasGhost*/) const {
   ...
   bool
   _isGhost(const const_iterator node, Loki::Int2Type<false> /*HasGhost*/)
   const {
      return false;
   }
   ...
   //! Return true if it is a ghost node.
   bool
   _isGhost(const const_iterator node, Loki::Int2Type<true> /*HasGhost*/)
   const {
#ifdef DEBUG_stlib
      assert(node != end());
#endif
   ...
   advance(iterator* node, _Distance n) {
      _advance(node, n, Loki::Int2Type<Traits::HasGhost>());
   }

   //! Advance the iterator.
   template<typename _Distance>
   void
   _advance(iterator* node, _Distance n, Loki::Int2Type<false> /*HasGhost*/) {
      std::advance(*node, n);
   }
   ...
   template<typename _Distance>
   void
   _advance(iterator* node, _Distance n, Loki::Int2Type<true> /*HasGhost*/) {
      if (n > 0) {
         while (n) {
            ++*node;
            if (! isGhost(*node)) {
...
inline
bool
hasNodesAtLevel(const Orthtree<_Patch, _Traits>& orthtree, const int level) {
