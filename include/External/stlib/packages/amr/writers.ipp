...
inline
void
writeElements(std::ostream& out, const std::tr1::array<_T, _N>& vector) {
   out << vector[0];
...
inline
void
writeElementsInterlaced(std::ostream& out, const std::tr1::array<_T, _N>& a,
                        const std::tr1::array<_T, _N>& b) {
   out << a[0] << " " << b[0];
   for (std::size_t n = 1; n != _N; ++n) {
      out << " " << a[n] << " " << b[n];
   }
...
writeCellDataVtkXml(std::ostream& out,
   ...
   writeElementsInterlaced
      (out,
       ext::convert_array<Number>(spatialIndex.getCoordinates() *
                                  array.extents()),
       ext::convert_array<Number>((spatialIndex.getCoordinates() + 1) *
                                  array.extents()));
   out << "\" Origin=\"";
   writeElements(out, orthtree.getLowerCorner());
   out << "\" Spacing=\"";
   writeElements(out, ext::convert_array<Number>
                 (orthtree.getExtents(spatialIndex) /
                  array.extents()));
   out << "\">\n";
