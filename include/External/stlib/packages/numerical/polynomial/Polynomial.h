...
/*! \defgroup numerical_polynomial_static Evaluating Polynomials
  ...
  There are two functions that accept a std::tr1::array of the coefficients.
  For this interface the order can be deduced from the size of the array.
  \code
  const std::tr1::array<double, 3> c = {{2, 3, 5}};
...
inline
_T
evaluatePolynomial(const std::tr1::array<_T, _Size>& coefficients,
                   const _T x) {
...
template<std::size_t _Order, typename _RandomAccessIterator, typename _T>
_T
evaluatePolynomial(_RandomAccessIterator coefficients, _T x, _T* derivative, _T* derivative2);
...
evaluatePolynomial(const std::tr1::array<_T, _Size>& coefficients,
   ...
   return evaluatePolynomial < _Size - 1 > (coefficients.begin(), x, derivative, derivative2);
}
...
private:
   ...
   std::tr1::array < _T, _Order + 1 > _coefficients;
...
public:
   ...
   //! Construct from the polynomial coefficients.
   Polynomial(const std::tr1::array < _T, _Order + 1 > & coefficients) :
...
public:
   ...
   //! Set the coefficients.
   void
   setCoefficients(const std::tr1::array < _T, _Order + 1 > & coefficients) {
      _coefficients = coefficients;
   }
...
constructPolynomial(const std::tr1::array<_T, _Size>& coefficients) {
