...
private:
   ...
   typedef std::tr1::array < point_type, N + 1 > point_container;
   ...
   std::tr1::array < point_type, N + 1 > _vertices;

   // The objective function values at the vertices.
   std::tr1::array < number_type, N + 1 > _values;
...
public:
   ...
   */
   ...
   //! Find the minimum to within the tolerance.
   bool
   find_minimum(const std::tr1::array < point_type, N + 1 > & vertices);
...
private:
   ...
   // Initialize the simplex.
   void
   initialize(const std::tr1::array < point_type, N + 1 > & vertices);
