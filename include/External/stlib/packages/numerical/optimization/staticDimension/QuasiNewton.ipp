...
find_minimum(point_type& x, number_type& value, std::size_t& num_iterations,
   ...
   if (max_step == 0.0) {
      max_step = MAX_STEP * std::max(magnitude(x),
                                     number_type(N));
   }
   ...
   for (num_iterations = 0; num_iterations != MAX_ITERATIONS;
      ...
      test = 0.0;
      for (std::size_t i = 0; i != N; ++i) {
         temp = std::abs(xi[i]) / std::max(std::abs(x[i]), 1.0);
         if (temp > test) {
            test = temp;
         }
      }
...
line_search(const point_type& x_old, const number_type f_old,
   ...
   const number_type length_direction = magnitude(direction);
   ...
   test = 0.0;
   for (std::size_t i = 0; i != N; ++i) {
      temp = std::abs(direction[i]) / std::max(std::abs(x_old[i]), 1.0);
      if (temp > test) {
         test = temp;
      }
   }
   ...
   */
   ...
   for (;;) {
      ...
      */
      ...
      lambda2 = lambda;
      f2 = f;
      lambda = std::max(temp_lambda, 0.1 * lambda);
   }
}
