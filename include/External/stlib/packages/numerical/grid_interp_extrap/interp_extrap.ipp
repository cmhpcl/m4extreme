...
intExt(FieldTuple* value,
       bool* isValueKnown,
       const std::tr1::array<FieldTuple, 4>& field,
       const std::tr1::array<bool, 4>& isKnown,
       const T position,
...
inline
void
makeFieldsTuple(std::tr1::array<std::tr1::array<T, M>, 4>* fieldsTuple,
                const array::MultiArrayConstRef < T, 1 + 1 > fields,
                array::MultiIndexTypes<1>::IndexList i) {
   typedef array::MultiIndexTypes < 1 + 1 >::Index Index;
   array::MultiIndexTypes < 1 + 1 >::IndexList mi;
   for (Index x = 0; x != 4; ++x) {
      mi[1] = i[0] + x;
...
inline
void
intExt(std::tr1::array<T, M>* values,
       const std::tr1::array<T, 1>& position,
       const std::tr1::array<T, M>& defaultValues,
       const geom::RegularGrid<1, T>& grid,
       const array::MultiArrayConstRef<T, 1> distance,
       const array::MultiArrayConstRef < T, 1 + 1 > fields) {
   // The single index type.
   typedef array::MultiIndexTypes<1>::Index Index;
   // Multi-index.
   typedef array::MultiIndexTypes<1>::IndexList IndexList;

   //
   // The position is surrounded by 4 grid points.
   // First calculate the index of the left point.
   //
   std::tr1::array<T, 1> continuousIndex = position;
   ...
   std::tr1::array<std::tr1::array<T, M>, 4> fieldsTuple;
   IndexList mi;
   mi[0] = i;
   makeFieldsTuple(&fieldsTuple, fields, mi);

   std::tr1::array<bool, 4> isKnownTuple;
...
void
makeFieldsTuple(std::tr1::array<std::tr1::array<T, M>, 4>* fieldsTuple,
                const array::MultiArrayConstRef < T, 2 + 1 > fields,
                array::MultiIndexTypes<2>::IndexList i) {
   typedef array::MultiIndexTypes < 2 + 1 >::Index Index;
   array::MultiIndexTypes < 2 + 1 >::IndexList mi;
   mi[2] = i[1];
   for (Index x = 0; x != 4; ++x) {
      mi[1] = i[0] + x;
      for (mi[0] = 0; mi[0] != M; ++mi[0]) {
...
void
intExt(std::tr1::array<T, M>* values,
       const std::tr1::array<T, 2>& position,
       const std::tr1::array<T, M>& defaultValues,
       const geom::RegularGrid<2, T>& grid,
       const array::MultiArrayConstRef<T, 2> distance,
       const array::MultiArrayConstRef < T, 2 + 1 > fields) {
   // The single index type.
   typedef array::MultiIndexTypes<2>::Index Index;
   // Multi-index.
   typedef array::MultiIndexTypes<2>::IndexList IndexList;

   //
   // The position lies in a square defined by 16 grid points.
   // First calculate the indices of the lower corner.
   //
   std::tr1::array<T, 2> continuousIndex = position;
   grid.convertLocationToIndex(&continuousIndex);
   const Index i = static_cast<Index>(std::floor(continuousIndex[0])) - 1;
   ...
   //
   std::tr1::array<std::tr1::array<T, M>, 4> xInterpFields;
   std::tr1::array<bool, 4> xInterpIsKnown;
   Index x, y;
   IndexList mi;
...
void
makeFieldsTuple(std::tr1::array<std::tr1::array<T, M>, 4>* fieldsTuple,
                const array::MultiArrayConstRef < T, 3 + 1 > fields,
                array::MultiIndexTypes<3>::IndexList i) {
   typedef array::MultiIndexTypes < 3 + 1 >::Index Index;
   array::MultiIndexTypes < 3 + 1 >::IndexList mi;
   mi[2] = i[1];
   mi[3] = i[2];
   for (Index x = 0; x != 4; ++x) {
      mi[1] = i[0] + x;
...
intExt(std::tr1::array<T, M>* values,
   ...
   // Interpolate in the x direction.
   //
   std::tr1::array<T, M> xInterpFields[4][4];
   ...
   // Interpolate in the y direction.
   //
   std::tr1::array<std::tr1::array<T, M>, 4> yInterpFields;
   std::tr1::array<bool, 4> yInterpIsKnown;
...
inline
void
gridInterpExtrap(std::vector<std::tr1::array<T, M> >* values,
                 const std::vector<std::tr1::array<T, N> >& positions,
                 const std::tr1::array<T, M>& defaultValues,
                 const geom::RegularGrid<N, T>& grid,
                 const array::MultiArrayConstRef<T, N> distance,
                 const array::MultiArrayConstRef < T, N + 1 > fields) {
...
gridInterpExtrap(std::vector<std::tr1::array<T, M> >* values,
                 const std::vector<std::tr1::array<T, N> >& positions,
                 const std::tr1::array<T, M>& defaultValues,
                 const std::tr1::array<std::size_t, N>& extents,
                 const std::tr1::array<T, N>& lowerCorner,
                 const std::tr1::array<T, N>& upperCorner,
                 const T* distance,
                 const T* fields) {
   //
   ...
   const array::MultiArrayConstRef<T, N> _distance(distance, extents);
   std::tr1::array < std::size_t, N + 1 > fieldExtents;
   fieldExtents[0] = M;
   std::copy(extents.begin(), extents.end(), fieldExtents.begin() + 1);
   const array::MultiArrayConstRef < T, N + 1 > _fields(fields, fieldExtents);
