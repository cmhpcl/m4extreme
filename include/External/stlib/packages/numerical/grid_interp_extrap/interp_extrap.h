...
gridInterpExtrap(const std::size_t size,
                 ...
                 const T* fields) {
   // Copy from the input.
   std::vector<std::tr1::array<T, M> > values(size);
   {
      T* v = valuesData;
   ...
   std::vector<std::tr1::array<T, N> > positions(size);
   ...
   const std::tr1::array<T, M> defaultValues =
      ext::copy_array<std::tr1::array<T, M> >(defaultValuesData);
   const std::tr1::array<std::size_t, N> extents =
      ext::copy_array<std::tr1::array<std::size_t, N> >(extentsData);
   const std::tr1::array<T, N> lowerCorner =
      ext::copy_array<std::tr1::array<T, N> >(lowerCornerData);
   const std::tr1::array<T, N> upperCorner =
      ext::copy_array<std::tr1::array<T, N> >(upperCornerData);
   // Interpolate.
