...
\c std::vector<double> and \c std::tr1::array<double,N> are suitable
...
double
derivativeRidders(_Functor& f, double x, double scale, double* error,
                  ads::SquareMatrix<10, double>* a);
...
inline
double
derivativeRidders(_Functor& f, const double x) {
   // Take a guess at an appropriate length scale over which the function
   // changes appreciably.
   const double scale = std::max(0.001 * std::abs(x), 0.001);
