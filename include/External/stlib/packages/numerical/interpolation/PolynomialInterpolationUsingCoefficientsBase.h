...
/*!
  ...
  with the DEBUG_stlib macro then trying to perform interpolation on 
  ...
  typedef array::MultiIndexRange<Dimension> ArrayRange;
  typedef array::MultiIndexRangeIterator<Dimension> ArrayIterator;
  // Make a grid to sample the exponential function on the domain [0..1).
  // Use a grid spacing of 0.1.
  std::vector<double> grid(11);
  const double Lower = 0;
...
protected:
   ...
   typedef std::tr1::array<_T, _Order+1> Coefficients;
...
#include "PolynomialInterpolationUsingCoefficientsBase.ipp"
