...
initialize(const ValueGrid& valueGrid, const BBox& domain,
   ...
   typedef array::MultiIndexRange<Base::Dimension> Range;
   typedef array::MultiIndexRangeIterator<Base::Dimension> Iterator;

   // There must be at least two grid points in each dimension.
   assert(min(valueGrid.extents()) >= std::size_t(2));
   ...
   ValueGrid extension(valueGrid.extents() + 2, ext::make_array<Index>(-1, -1));
...
initialize(const ValueGrid& valueGrid, const BBox& domain,
   ...
   assert(min(valueGrid.extents()) >= std::size_t(1));

   // Allocate memory for the grid.
   _grid.rebuild(valueGrid.extents() + 1);
   ...
   ValueGrid extension(valueGrid.extents() + 3, ext::make_array<Index>(-1, -1));
   const SizeList& e = valueGrid.extents();
   const array::MultiIndexRange<Base::Dimension> range = extension.range();
   const Iterator end = Iterator::end(range);
   for (Iterator i = Iterator::begin(range); i != end; ++i) {
...
initialize(const ValueGrid& g) {
   ...
   assert(_grid.extents() + 2 == g.extents());
...
interpolate(Point x, ValueGradientHessian* vgh,
   ...
   // Interpolate in the x direction along the line y = 0.
   {
      ...
      interpolateX(a, b, x[0], &_y0, Loki::Int2Type<5>());
   }
   ...
   // Interpolate in the x direction along the line y = 1.
   {
      ...
      _cell[0] += 1;
      const Dt2d3& b = _grid(_cell);
      interpolateX(a, b, x[0], &_y1, Loki::Int2Type<5>());
   }

   // Interpolate in the y direction.
   interpolateY(_y0, _y1, x[1], vgh, Loki::Int2Type<5>());
...
interpolate(Point x, ValueGradientHessian* vgh,
   ...
   // Interpolate in the x direction along the line y = 0.
   {
      ...
      _cell[0] += 1;
      const Dt2d3& b = _grid(_cell);
      interpolateX(a, b, x[0], &_y0, Loki::Int2Type<3>());
   }
   ...
   // Interpolate in the x direction along the line y = 1.
   {
      ...
      _cell[0] += 1;
      const Dt2d3& b = _grid(_cell);
      interpolateX(a, b, x[0], &_y1, Loki::Int2Type<3>());
   }

   // Interpolate in the y direction.
   interpolateY(_y0, _y1, x[1], vgh, Loki::Int2Type<3>());
...
interpolate(Point x, ValueGradientHessian* vgh,
   ...
   // Interpolate in the x direction along the line y = 0.
   {
      ...
      _cell[0] += 1;
      const Dt2d3& b = _grid(_cell);
      interpolateX(a, b, x[0], &_y0, Loki::Int2Type<1>());
   }
   ...
   // Interpolate in the x direction along the line y = 1.
   {
      ...
      _cell[0] += 1;
      const Dt2d3& b = _grid(_cell);
      interpolateX(a, b, x[0], &_y1, Loki::Int2Type<1>());
   }

   // Interpolate in the y direction.
   interpolateY(_y0, _y1, x[1], vgh, Loki::Int2Type<1>());
...
interpolateX(const Dt2d3& a, const Dt2d3& b, const Value t,
             std::tr1::array<Dt1d3, 3>* f, Loki::Int2Type<5> /*order*/) const {
   // For f, f_y, and f_yy.
...
void
QuinticInterpolation2D<_T, _IsPeriodic>::
interpolateY(const std::tr1::array<Dt1d3, 3>& a,
             const std::tr1::array<Dt1d3, 3>& b,
...
QuinticInterpolation2D<_T, _IsPeriodic>::
interpolateX(const Dt2d3& a, const Dt2d3& b, const Value t,
             std::tr1::array<Dt1d3, 3>* f, Loki::Int2Type<3> /*order*/) const {
   // For f and f_y.
...
QuinticInterpolation2D<_T, _IsPeriodic>::
interpolateX(const Dt2d3& a, const Dt2d3& b, const Value t,
             std::tr1::array<Dt1d3, 3>* f, Loki::Int2Type<1> /*order*/) const {
