...
lin_interp(const array::MultiArray<F, 1>& fields,
           typename array::MultiArray<F, 1>::IndexList i,
           const std::tr1::array<T, 1>& t) {
...
template<typename F, typename T>
typename Loki::TypeTraits<F>::ParameterType
lin_interp(const array::MultiArray<F, 2>& fields,
           typename array::MultiArray<F, 2>::IndexList i,
           const std::tr1::array<T, 2>& t) {
   // Make these static in case F is not a fundamental type.
   static F f, g;
#ifdef DEBUG_stlib
...
template<typename F, typename T>
typename Loki::TypeTraits<F>::ParameterType
lin_interp(const array::MultiArray<F, 3>& fields,
           typename array::MultiArray<F, 3>::IndexList i,
           const std::tr1::array<T, 3>& t) {
   // Make these static in case F is not a fundamental type.
   static F f, g;
#ifdef DEBUG_stlib
   assert(0 <= t[0] && t[0] <= 1 && 0 <= t[1] && t[1] <= 1 &&
