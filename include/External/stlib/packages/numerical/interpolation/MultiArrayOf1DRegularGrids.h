...
private:
   ...
   typedef array::MultiIndexTypes<_Dimension> Types;
...
public:
   ...
   (const std::size_t gridSize, const SizeList& arrayExtents) :
      _data((gridSize + 2 * GhostWidth) * product(arrayExtents)),
      _gridSize(gridSize),
      _arrayExtents(arrayExtents),
      _strides() {
      LOKI_STATIC_CHECK(_Order == 1 || _Order == 3, NotSupported);
      assert(! _data.empty());
      computeStrides();
   }
...
public:
   ...
   gridData(const IndexList& indices) const {
      return &_data[dot(_strides, indices) * (_gridSize + 2 * GhostWidth)]
         + GhostWidth;
   }
