...
protected:
   ...
   const std::tr1::array<_T, 3> _offsets;
...
public:
   ...
   QuinticInterpolation2DConstants() :
      ...
      _offsets(ext::make_array<_T>(-_delta, 0, _delta)) {
   }

   //! Assignment operator.
   QuinticInterpolation2DConstants&
   operator=(const QuinticInterpolation2DConstants& other) {
      // There is only static const and const data.
      return *this;
   }
};
...
public:
   ...
   //! A Cartesian point.
   typedef std::tr1::array<_T, Base::Dimension> Point;
   //! A bounding box.
   typedef geom::BBox<_T, Base::Dimension> BBox;
   //! The %array of function values.
   typedef array::MultiArray<Value, Base::Dimension> ValueGrid;
...
protected:
   ...
   mutable std::tr1::array<Value, 6> _c;
   //! Used for interpolation in the x dimension.
   mutable std::tr1::array<Dt1d3, 3> _y0, _y1;
...
public:
   ...
   */
   ...
   initialize(const ValueGrid& valueGrid, const BBox& domain) {
      initialize(valueGrid, domain, Loki::Int2Type<_IsPeriodic>());
   }
...
public:
   ...
   operator()(const Point& x, ValueGradientHessian* vgh) const {
      interpolate(x, vgh, Loki::Int2Type<5>());
   }
   ...
   interpolate(const Point& x) const {
      ...
      interpolate(x, &vgh, Loki::Int2Type<_Order>());
      return vgh.f;
   }
   ...
   interpolate(const Point& x,
               std::tr1::array<Value, Base::Dimension>* gradient) const {
      ValueGradientHessian vgh;
      interpolate(x, &vgh, Loki::Int2Type<_Order>());
      (*gradient)[0] = vgh.fx;
      (*gradient)[1] = vgh.fy;
...
protected:
   ...
   interpolateX(const Dt2d3& a, const Dt2d3& b, const Value t,
                std::tr1::array<Dt1d3, 3>* f, Loki::Int2Type<3> /*order*/)
      const;
   ...
   void
   interpolateX(const Dt2d3& a, const Dt2d3& b, const Value t,
                std::tr1::array<Dt1d3, 3>* f, Loki::Int2Type<1> /*order*/)
      const;
...
#include "QuinticInterpolation2D.ipp"
