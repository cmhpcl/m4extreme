...
/*!
  ...
  typedef array::MultiIndexRangeIterator<Dimension> Iterator;
  ...
  typedef InterpolatingFunction::Point Point;
  typedef InterpolatingFunction::BBox BBox;
  typedef array::MultiIndexRange<ArrayDimension> ArrayRange;
  typedef array::MultiIndexRangeIterator<ArrayDimension> ArrayIterator;
  typedef array::MultiIndexRange<GridDimension> GridRange;
  typedef array::MultiIndexRangeIterator<GridDimension> GridIterator;
...
class InterpolatingFunctionRegularGridBase :
   ...
   // value type.
   public std::unary_function<std::tr1::array<double, _Dimension>, _T> {
   //
   // Types.
   //
private:
   ...
   //! The functor type.
   typedef std::unary_function<std::tr1::array<double, _Dimension>, _T>
   Functor;
...
public:
   ...
   typedef _T Value;
   //! A Cartesian point.
   typedef std::tr1::array<double, _Dimension> Point;
   //! A bounding box.
   typedef geom::BBox<double, _Dimension> BBox;
   //! The %array of function values.
   typedef array::MultiArrayConstRef<Value, _Dimension> GridConstRef;
   //! The (multi) size type.
...
public:
   ...
   snapToGrid(const argument_type& x, IndexList* index) const {
      ...
      calculateScaledCoordinates(index, &y,
                                 Loki::Int2Type<true>() /*point-based*/);
   }
...
private:
   ...
   calculateScaledCoordinates(IndexList* cell, argument_type* x) const {
      calculateScaledCoordinates(cell, x, Loki::Int2Type<_Order == 0>());
   }
...
#include "InterpolatingFunctionRegularGridBase.ipp"
