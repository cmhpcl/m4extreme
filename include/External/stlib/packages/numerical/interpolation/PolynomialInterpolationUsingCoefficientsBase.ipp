...
interpolate(argument_type x) const {
   LOKI_STATIC_CHECK(_InterpolationOrder <= _Order, NotSupported);
...
#endif
   x -= i;
   return interpolate(x, i, Loki::Int2Type<_Order>());
}
...
interpolate(argument_type x, Value* derivative) const {
   LOKI_STATIC_CHECK(_InterpolationOrder <= _Order, NotSupported);
   LOKI_STATIC_CHECK(_Order >= 3, NotSupported);
   // Convert to index coordinates.
   x = _inverseWidth * (x - _lowerCorner);
#ifdef DEBUG_stlib
   assert(x >= 0);
...
#endif
   x -= i;
   return interpolate(x, i, derivative, Loki::Int2Type<_Order>());
}
...
interpolate(argument_type x, Value* firstDerivative, Value* secondDerivative)
   const {
   LOKI_STATIC_CHECK(_InterpolationOrder == _Order, NotSupported);
   LOKI_STATIC_CHECK(_Order == 5, NotSupported);
   // Convert to index coordinates.
   x = _inverseWidth * (x - _lowerCorner);
#ifdef DEBUG_stlib
   assert(x >= 0);
...
setGridValues(_ForwardIterator f) {
   ...
   setCoefficients(&fg[2], Loki::Int2Type<_Order>());
}
...
setGridValues(_ForwardIterator f, _ForwardIterator df) {
   ...
   // Set the polynomial coefficients.
   setCoefficients(&fg[1], &dfg[1], Loki::Int2Type<_Order>());
}
...
setGridValues(_ForwardIterator f, _ForwardIterator df,
   ...
   // Set the polynomial coefficients.
   setCoefficients(&fg[1], &dfg[1], &ddfg[1], Loki::Int2Type<_Order>());
#endif
   //
   ...
   // Set the polynomial coefficients.
   setCoefficients(&fg[0], &dfg[0], &ddfg[0], Loki::Int2Type<_Order>());
}
...
PolynomialInterpolationUsingCoefficientsBase<_T, _Order>::
interpolate(const argument_type x, const std::size_t i,
            Value* derivative, Loki::Int2Type<3> /*Order*/) const {
   const Coefficients& c = _coefficients[i];
...
PolynomialInterpolationUsingCoefficientsBase<_T, _Order>::
interpolate(const argument_type x, const std::size_t i,
            Value* derivative, Loki::Int2Type<5> /*Order*/) const {
   const Coefficients& c = _coefficients[i];
   *derivative = c[1]
...
void
PolynomialInterpolationUsingCoefficientsBase<_T, _Order>::
setCoefficients(const Value* f, Loki::Int2Type<1> /*Order*/) {
...
void
PolynomialInterpolationUsingCoefficientsBase<_T, _Order>::
setCoefficients(const Value* f, Loki::Int2Type<3> /*Order*/) {
   for (std::ptrdiff_t i = 0; i != std::ptrdiff_t(_numberOfCells); ++i) {
      Coefficients& c = _coefficients[i];
      c[0] = f[i];
      c[1] = 0.5 * (- f[i-1] + f[i+1]);
...
void
PolynomialInterpolationUsingCoefficientsBase<_T, _Order>::
setCoefficients(const Value* f, Loki::Int2Type<5> /*Order*/) {
   for (std::ptrdiff_t i = 0; i != std::ptrdiff_t(_numberOfCells); ++i) {
      Coefficients& c = _coefficients[i];
#if 0
      c[0] = f[i];
...
setCoefficients(const Value* f, const Value* /*df*/,
                Loki::Int2Type<1> /*Order*/) {
   // Ignore the derivative information.
   setCoefficients(f, Loki::Int2Type<1>());
}
...
setCoefficients(const Value* f, const Value* df, const Value* /*ddf*/,
   ...
   setCoefficients(f, df, Loki::Int2Type<3>());
}
...
PolynomialInterpolationUsingCoefficientsBase<_T, _Order>::
checkErrors() const {
   // CONTINUE Remove this macro. The check should always be performed.
#ifdef DEBUG_stlib
   ...
   for (std::size_t i = 0; i != _numberOfCells - 1; ++i) {
      const result_type f = interpolate(argument_type(1), i,
                                        Loki::Int2Type<_Order>());
