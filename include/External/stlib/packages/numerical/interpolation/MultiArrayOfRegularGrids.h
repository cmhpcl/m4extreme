...
private:
   ...
   typedef array::MultiIndexTypes<_ArrayDimension> ArrayMultiIndex;
...
public:
   ...
   typedef array::MultiArrayRef<_T, _GridDimension> GridRef;
   //! A const reference to a grid.
   typedef array::MultiArrayConstRef<_T, _GridDimension> GridConstRef;
...
public:
   ...
   MultiArrayOfRegularGrids(const GridSizeList& gridExtents,
      ...
      _data(product(gridExtents) * product(arrayExtents)),
...
public:
   ...
   gridData(const ArrayIndexList& indices) const {
      return &_data[dot(_strides, indices) * _gridConstRef.size()];
   }
...
public:
   ...
   gridData(const ArrayIndexList& indices) {
      return &_data[dot(_strides, indices) * _gridRef.size()];
   }
