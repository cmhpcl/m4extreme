...
inline
_T
distance(const IntersectionPoint<_T, _D>& p, const std::tr1::array<_T, _D>& x) {
   // Determine the sign of the distance.
   std::tr1::array<_T, _D> v = p.location;
   v -= x;
...
makeBoundaryIntersection(const geom::Ball<_T, 2>& a, const geom::Ball<_T, 2>& b,
   ...
   const _T d2 = squaredDistance(a.center, b.center);
   ...
   std::tr1::array<_T, 2> offset = p->normal;
   offset *= h;
   q->location -= offset;
   q->normal = p->normal;
   negateElements(&q->normal);
...
makeBoundaryIntersection(const geom::Ball<_T, 3>& a, const geom::Ball<_T, 3>& b,
   ...
   typedef std::tr1::array<_T, D> Point;
   ...
   const _T d2 = squaredMagnitude(e[0]);
   if (d2 * (1. + std::numeric_limits<_T>::epsilon()) >= 
       (a.radius + b.radius) * (a.radius + b.radius)) {
      return false;
   }
   ...
   const _T d = std::sqrt(d2);
   e[0] /= d;
   const _T i = dot(e[0], c.center - a.center);
   // y-axis.
   e[1] = c.center - a.center - i * e[0];
   normalize(&e[1]);
   const _T j = dot(e[1], c.center - a.center);
   // z-axis.
   e[2] = cross(e[0], e[1]);
...
inline
void
boundNegativeDistance(const IntersectionPoint<_T, 2>& p, geom::BBox<_T, 2>* box) {
   ...
   std::tr1::array<Point, 4> corners = {{p.location, p.location, p.location,
                                         p.location}};
   // 0 1
   // 2 3
   ...
   corners[1] -= offset;
   corners[3] -= offset;
   box->bound(corners.begin(), corners.end());
}
...
inline
void
boundNegativeDistance(const IntersectionPoint<_T, 3>& p, geom::BBox<_T, 3>* box) {
   ...
   normalize(&x);
   Point y = cross(p.normal, x);
   x *= std::sqrt(2.) * p.radius;
   y *= std::sqrt(2.) * p.radius;
