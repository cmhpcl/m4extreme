...
distance(const std::vector<geom::Ball<_T, _D> >& balls,
         ...
         const std::tr1::array<_T, _D>& x) {
   std::tr1::array<_T, _D> closest;
...
negativeDistance(const std::vector<geom::Ball<_T, _D> >& balls,
                 const std::size_t n, const std::tr1::array<_T, _D>& x) {
   std::tr1::array<_T, _D> closest;
   const _T d = closestPoint(balls[n], x, &closest);
   if (d > 0) {
      return std::numeric_limits<_T>::infinity();
...
negativeDistance(array::SimpleMultiArrayRef<_T, 2>* grid,
   ...
   std::tr1::array<IntersectionPoint<_T, D>, 2> pointsOnSurface;
   geom::Ball<_T, 2> pointBall;
   std::tr1::array<_T, D> v;
   for (std::size_t i = 0; i != balls.size(); ++i) {
...
template<typename _T, std::size_t _N, typename _Base>
inline
void
negativeDistance(array::EquilateralArrayImp<_T, 2, _N, _Base>* patch,
   ...
   for (Iterator i = Iterator::begin(patch->extents()); i != end; ++i) {
      ...
      for (std::size_t n = 0; n != points.size(); ++n) {
         ...
         d = squaredDistance(p, pos.location);
         if (d < g) {
            g = d;
         }
      }
...
inline
void
negativeDistance(array::SimpleMultiArrayRef<_T, 3>* grid,
         ...
         for (std::size_t k = j + 1; k != balls.size(); ++k) {
            ...
            for (std::size_t m = 0; m != pointsOnSurface.size(); ++m) {
               ...
               for (Iterator index = Iterator::begin(range); index != end;
                  ...
                  const _T sd = squaredDistance(p, pos.location);
                  if (sd > squaredRadius) {
                     continue;
                  }
                  // Ignore points with positive distance.
...
updateDistance(const std::vector<geom::Ball<_T, 3> >& balls,
               ...
               const std::tr1::array<_T, 3>& x, _T* squaredDist) {
   // Ignore points that are not inside one of the two balls.
...
void
negativeDistance
(array::EquilateralArrayImp<_T, 3, _N, _Base>* patch,
...
inline
void
negativeDistance(array::EquilateralArrayImp<_T, 3, _N, _Base>* patch,
                 const std::tr1::array<_T, 3>& lowerCorner,
                 const _T spacing,
                 const std::vector<geom::Ball<_T, 3> >& balls) {
   const std::size_t D = 3;
