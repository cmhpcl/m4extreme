...
GridVirtual(const BBox& domain, const _T targetSpacing) :
   ...
   spacing(max((domain.upper - domain.lower) / (extents * (_N - 1)))),
   _strides(calculateStrides(extents)) {
   LOKI_STATIC_CHECK(_N > 1, BadPatchExtent);
...
template<typename _T, std::size_t _D, std::size_t _N>
inline
typename GridVirtual<_T, _D, _N>::IndexList
GridVirtual<_T, _D, _N>::
calculateExtents(const BBox& domain, const _T targetSpacing) {
   // Number of cells = ext * (_N - 1).
   IndexList e;
   for (std::size_t i = 0; i != e.size(); ++i) {
      ...
      // length = dx * e * (_N - 1)
      // e = length / (dx * (_N - 1))
      e[i] = std::size_t(std::ceil(length / (targetSpacing * (_N - 1))));
      assert(e[i] != 0);
   }
   return e;
}
...
template<typename _T, std::size_t _D, std::size_t _N>
inline
typename GridVirtual<_T, _D, _N>::BBox
GridVirtual<_T, _D, _N>::
domain() const {
   BBox domain = {lowerCorner,
                  lowerCorner + extents * (_N - 1) * spacing};
   return domain;
}
...
template<typename _T, std::size_t _D, std::size_t _N>
inline
typename GridVirtual<_T, _D, _N>::Point
GridVirtual<_T, _D, _N>::
getPatchLowerCorner(const IndexList& i) const {
   return lowerCorner + getPatchLength() * i;
}
...
patchDependencies(const GridVirtual<_T, _D, _N>& grid, _InputIterator begin,
   ...
   typedef GridVirtual<_T, _D, _N> GridVirtual;
   ...
   std::vector<std::size_t> sizes(product(grid.extents), 0);
