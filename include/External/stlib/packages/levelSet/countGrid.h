...
inline
bool
hasKnown(const Grid<_T, _D, _N>& grid) {
...
inline
bool
hasUnknown(const Grid<_T, _D, _N>& grid) {
   typedef typename Grid<_T, _D, _N>::VertexPatch VertexPatch;
   // Loop over the patches.
   for (std::size_t i = 0; i != grid.size(); ++i) {
      const VertexPatch& p = grid[i];
...
inline
bool
hasUnknownAdjacentNeighbor(const Grid<_T, _D, _N>& grid,
                           const typename Grid<_T, _D, _N>::DualIndices& ij) {
...
inline
bool
hasNegativeAdjacentNeighbor(const Grid<_T, _D, _N>& grid,
                            const typename Grid<_T, _D, _N>::DualIndices& ij) {
   typedef Grid<_T, _D, _N> Grid;
   typedef typename Grid::DualIndices DualIndices;
...
inline
void
printLevelSetInfo(const Grid<_T, _D, _N>& grid, std::ostream& out) {
   typedef typename Grid<_T, _D, _N>::VertexPatch VertexPatch;
