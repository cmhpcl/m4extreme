...
class GridUniform :
      public array::SimpleMultiArray<_T, _D> {
   //
   // Constants.
   //
public:
...
private:
   ...
   typedef array::SimpleMultiArray<_T, _D> Base;
...
public:
   ...
   */
   GridUniform(const BBox& domain,
      ...
      spacing(max((domain.upper - domain.lower) / (Base::_extents - 1))) {
...
public:
   ...
   domain() const {
      BBox domain = {lowerCorner,
                     lowerCorner + (Base::extents() - 1) * spacing};
      return domain;
   }
