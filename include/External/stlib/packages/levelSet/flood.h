...
inline
void
floodFillInterval(array::SimpleMultiArrayRef<_T, _D>* f,
   ...
   const IndexList closedUpper = f->extents() - 1;
...
inline
void
floodFillInterval(Grid<_T, _D, _N>* f,
                  const _T lowerThreshold, const _T upperThreshold,
                  const _T lowerFill = -std::numeric_limits<_T>::infinity(),
                  const _T upperFill = std::numeric_limits<_T>::infinity()) {
   ...
   // Pop values until the stack is empty.
   IndexList i;
   IndexList center = ext::filled_array<IndexList>(_N / 2);
   const _T mean = 0.5 * (lowerThreshold + upperThreshold);
   while (! known.empty()) {
      i = known.front();
      known.pop_front();
...
inline
void
floodFill(array::SimpleMultiArrayRef<_T, _D>* f,
