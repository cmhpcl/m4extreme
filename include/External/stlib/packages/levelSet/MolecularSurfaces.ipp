...
solventExcludedCavities(_OutputIterator vertices) const {
   ...
   array::StaticArrayOfArrays<std::size_t> offsetDependencies;
...
template<typename _T, std::size_t _D, std::size_t _N>
inline
typename MolecularSurfaces<_T, _D, _N>::BBox
MolecularSurfaces<_T, _D, _N>::
calculateDomain(const std::vector<geom::Ball<_T, _D> >& balls) const {
   ...
   domain.offset(targetGridSpacing);
   return domain;
}
