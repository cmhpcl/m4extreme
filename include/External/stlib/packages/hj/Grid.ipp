...
Grid(array::MultiArrayRef<Number, N>& solution, const Number dx,
   ...
   // scheme.
   _ranges(_solution.extents() + (is_concurrent ?
                                  2 * DifferenceScheme::radius() : 0),
           _solution.bases() - (is_concurrent ?
...
void
Grid<N, T, DifferenceScheme>::
add_source(const std::tr1::array<Number, 2>& x) {
...
void
Grid<N, T, DifferenceScheme>::
add_source(const std::tr1::array<Number, 3>& x) {
...
print_solution_array(std::ostream& out,
                     const array::MultiArrayConstRef<T, 2>& solution) {
   typedef array::MultiArrayConstRef<T, 2> MultiArray;
...
void
print_solution_array(std::ostream& out,
                     const array::MultiArrayConstRef<T, 3>& solution) {
   typedef array::MultiArrayConstRef<T, 3> MultiArray;
   typedef typename MultiArray::IndexList IndexList;
   typedef typename MultiArray::Index Index;
