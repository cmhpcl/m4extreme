...
public:
   ...
   typedef array::MultiArrayRef<Number, N> NumberArrayRef;
...
public:
   ...
   /*!
     ...
     solver.
   */
   Grid(array::MultiArrayRef<Number, N>& solution, Number dx,
...
private:
   ...
   Number
   index_distance(const IndexList& i, const std::tr1::array<Number, 2>& x)
      const {
   ...
   //! Return the distance with coordinates given in index space.
   Number
   index_distance(const IndexList& i, const std::tr1::array<Number, 3>& x)
      const {
      return std::sqrt((i[0] - x[0]) *(i[0] - x[0]) + (i[1] - x[1]) *(i[1] - x[1]) +
                       (i[2] - x[2]) *(i[2] - x[2])) * _dx;
   }
