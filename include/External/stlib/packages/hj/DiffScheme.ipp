...
DiffScheme(const Range& index_ranges,
           array::MultiArrayRef<Number, N>& solution,
   ...
   _status(_index_ranges.extents() + (is_concurrent ? 4 : 2),
           _index_ranges.bases() - (is_concurrent ? 2 : 1)),
...
print_status_array(std::ostream& out,
                   const array::MultiArrayConstRef<Status, 2>& status) {
   typedef array::MultiArrayConstRef<Status, 2>::Index Index;
   typedef array::MultiArrayConstRef<Status, 2>::IndexList IndexList;
   out << "Status:" << '\n';
...
void
print_status_array(std::ostream& out,
                   const array::MultiArrayConstRef<Status, 3>& status) {
   typedef array::MultiArrayConstRef<Status, 3>::Index Index;
   typedef array::MultiArrayConstRef<Status, 3>::IndexList IndexList;
